        ...
        boost::asio::async_write(
            ...
            [this, self](boost::system::error_code ec, std::size_t length) {
                if (!ec) {
                    for (const Query &q : query_)
                        if (!q.host.empty())
                            std::make_shared<Client>(self, q.host, q.port,
                                                     q.file, q.session)
                                ->connect();
                }
            });
...
<body class="bg-secondary pt-5">
    ...
    class Client : public std::enable_shared_from_this<Client> {
       public:
        Client(std::shared_ptr<Session> session, std::string host,
               std::string port, std::string filename, std::string sname)
            : session_(session),
              resolv_(ioservice),
              socket_(ioservice),
              host_(host),
